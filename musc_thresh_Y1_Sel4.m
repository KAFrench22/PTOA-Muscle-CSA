...
%#######################################################################
%
%          * MUSCle THRESHold Program, Y1, Select 4 Muscle Areas *
%
...
%             mthresh.xlsx in the Muscle CSA_Y1 folder in the 
...
%             using this program.
%
%             8.  This program allows you to pick a point within FOUR 
%             separate muscle areas once the image has undergone 
%             thresholding ("bulk","gracilis","isolated1","isolated2").
%             This enables the program to include all muscle area within  
%             the polygon ROI, even if a part of the muscle is completely 
%             surrounded by fat. If you need to include more than four 
%             areas of muscle, you will need to edit the code to include  
%             an additional area of isolated muscle. 
%
...
uiwait(msgbox('Pick a point within the first left isolated muscle.', ...
    'Input','modal'));
[x3,y3] = ginput(1);
x3 = round(x3);
y3 = round(y3);

uiwait(msgbox('Pick a point within the second left isolated muscle.', ...
    'Input','modal'));
[x4,y4] = ginput(1);
x4 = round(x4);
y4 = round(y4);
...
bwml3 = grayconnected(imgbwl,y3,x3,1);  % Include connected muscle
bwml4 = grayconnected(imgbwl,y4,x4,1);  % Include connected muscle
%
bwml = bwml | bwml2 | bwml3 | bwml4;
imgml = imgt;
imgml(~bwml) = 0;
...
uiwait(msgbox('Pick a point within the first right isolated muscle.', ...
    'Input','modal'));
[xr3,yr3] = ginput(1);
xr3 = round(xr3);
yr3 = round(yr3);

uiwait(msgbox('Pick a point within the second right isolated muscle.', ...
    'Input','modal'));
[xr4,yr4] = ginput(1);
xr4 = round(xr4);
yr4 = round(yr4);
...
bwmr3 = grayconnected(imgbwr,yr3,xr3,1);  % Include connected muscle
bwmr4 = grayconnected(imgbwr,yr4,xr4,1);  % Include connected muscle

bwmr = bwmr | bwmr2 | bwmr3 | bwmr4;
imgmr = imgt;
imgmr(~bwmr) = 0;
