...
%#######################################################################
%
%           * MUSCle THRESHold Program, Select 3 Muscle Areas *
%
...
%             mthresh.xlsx in the Muscle CSA_Baseline folder in the 
...
%             using this program.
%
%             8.  This program allows you to pick a point within THREE 
%             separate muscle areas once the image has undergone 
%             thresholding ("bulk","gracilis", "isolated other"). This 
%             enables the program to include all muscle area within the 
%             polygon ROI, even if a part of the muscle is completely 
%             surrounded by fat. If you need to include more than three 
%             areas of muscle, please use the version of this M-file for 
%             four muscle areas. 
%
...
d = dir(path);  
rdir = 'Muscle CSA_Baseline';  % Results folder
...
%%
% Get T1 FFE Image File Name
ImageName = fnam(1:6);      % May need to update depending on file name
...
uiwait(msgbox('Pick a point within other left isolated muscle.', ...
    'Input','modal'));
[x3,y3] = ginput(1);
x3 = round(x3);
y3 = round(y3);
...
uiwait(msgbox('Pick a point within other right isolated muscle.', ...
    'Input','modal'));
[xr3,yr3] = ginput(1);
xr3 = round(xr3);
yr3 = round(yr3);
